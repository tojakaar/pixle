...
- **iOS Photos linker symbol:** Rust references `pixle_save_image_to_photos`; Swift exports it with `@_cdecl`. The iOS cdylib link allows that symbol as undefined in `src-tauri/build.rs` (`-Wl,-U,…`) because Swift is compiled later; after `tauri ios init`, run `npm run ios:sync-bridge` (or `npm run ios:dev`) so `PhotosBridge.swift` is a member of `pixle_iOS` for the final app link. Custom template: `src-tauri/ios-project.yml`. Do not stub the extern to silence linker errors.
