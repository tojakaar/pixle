...
- **AI editing API key:** Conversational edits call an OpenAI-compatible Chat Completions API from Rust (`src-tauri/src/ai_editor.rs`). Put `OPENAI_API_KEY` in a gitignored project-root **`.env`** (copy from `.env.example`). The app does **not** load `.env.example`. Optional: `OPENAI_BASE_URL` (default `https://api.openai.com/v1`), `OPENAI_MODEL` (default `gpt-4o-mini`). Never put the key in frontend code or Vite `VITE_*` vars.
