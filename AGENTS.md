...
- **AI editing API key:** Conversational edits use Rust (`src-tauri/src/ai_editor.rs`). Put keys in project-root **`.env`** (copy from `.env.example`; the app does **not** load `.env.example`). **Claude:** `ANTHROPIC_API_KEY` (+ optional `ANTHROPIC_MODEL`, default `claude-sonnet-4-20250514`). **OpenAI-compatible:** `OPENAI_API_KEY` (+ optional `OPENAI_BASE_URL` / `OPENAI_MODEL`). If both keys exist, Anthropic is preferred unless `LLM_PROVIDER=openai`. Never put keys in frontend / `VITE_*` vars.
