...
- **AI editing API key:** Conversational edits call an OpenAI-compatible Chat Completions API from Rust (`src-tauri/src/ai_editor.rs`). Set `OPENAI_API_KEY` in the environment, or in a gitignored `.env` / `src-tauri/.env` (see `.env.example`). Optional: `OPENAI_BASE_URL` (default `https://api.openai.com/v1`), `OPENAI_MODEL` (default `gpt-4o-mini`). The key must never be placed in frontend code or Vite `VITE_*` vars.
...
- **Large photos on macOS:** Images are decoded into a working buffer capped at 2048px on the long edge (`src/engine/imageDecode.ts`) so WKWebView does not freeze on multi‑megapixel camera JPEGs. Analysis reports the original file dimensions.
